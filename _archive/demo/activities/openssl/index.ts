...
export const opensslActions: FullConfigActions = {
  //"ensureFile": ["$ensureFile", [ "${dir}/${files.ca_key}" ]],
  //"ensureNoFile": ["$ensureNoFile", [ "${dir}/${files.ca_key}" ]],
  //"ensureNoFile": ["$ensureNoFile", [ "${dir}/${files.ca_key}" ]],

  "gen_ca_private_key": ['series',
    ["$print", "* Generate CA private key"],
    ["$exec", "openssl genrsa -aes256 -out \"${dir}/${files.ca_key}\" -passout pass:${password} 4096"],
    ["$ensureFile", "${dir}/${files.ca_key}"],
  ],
  "gen_ca_public_key": ['series',
    ["$print", "* Generate CA public key"],
    ["$exec", "openssl req -new -x509 -days \"${validity_days}\" -key \"${dir}/${files.ca_key}\" -sha256 -out \"${dir}/${files.ca}\" -subj \"/C=${country}/ST=${state}/L=${locality}/O=${organization}/OU=${organizationalunit}/CN=${commonname}/emailAddress=${email}\" -passin pass:${password}"],
  ...
  "gen_server_key": ['series',
    ...
    ["$exec", "openssl genrsa -out \"${dir}/${files.server_key}\" 4096"],
    ["$ensureFile", "${dir}/${files.server_key}"],
  ],
  "gen_server_csr": ['series',
    ["$print", "* Create server certificate signing request (CSR)"],
    ["$exec", "openssl req -subj \"/CN=${commonname}\" -sha256 -new -key \"${dir}/${files.server_key}\" -out \"${dir}/${files.server_csr}\""],
    ["$ensureFile", "${dir}/${files.server_csr}"],
  ],
  "write_server_ext": ['series',
    ["$print", "* Create a server ext file"],
    ["$exec", "node -e \"const fs=require('fs'); fs.writeFileSync('${dir}/${files.server_ext}', '${serverExtFile}', {encoding:'utf8'});\""],
    ["$ensureFile", "${dir}/${files.server_ext}"],
  ],
  "sign_server_public": ['series',
    ["$print", "* Sign server public key with our CA"],
    ["$exec", "openssl x509 -req -days \"${validity_days}\" -sha256 -in \"${dir}/${files.server_csr}\" -CA \"${dir}/${files.ca}\" -CAkey \"${dir}/${files.ca_key}\" -CAcreateserial -out \"${dir}/${files.server_cert}\" -extfile \"${dir}/${files.server_ext}\"  -passin pass:${password}"],
  ...
  "gen_client_key": ['series',
    ...
    ["$exec", "openssl genrsa -out \"${dir}/${files.client_key}\" 4096"],
    ["$ensureFile", "${dir}/${files.client_key}"],
  ],
  "gen_client_csr": ['series',
    ["$print", "* Create a client certificate signing request"],
    ["$exec", "openssl req -subj \"/CN=client\" -new -key \"${dir}/${files.client_key}\" -out \"${dir}/${files.client_csr}\""],
    ["$ensureFile", "${dir}/${files.client_csr}"],
  ],
  "write_client_ext": ['series',
    ["$print", "* Create a client ext file"],
    ["$exec", "node -e \"const fs=require('fs'); fs.writeFileSync('${dir}/${files.client_ext}', '${clientExtFile}', {encoding:'utf8'});\""],
    ["$ensureFile", "${dir}/${files.server_ext}"],
  ],
  "sign_client_public": ['series',
    ["$print", "* Sign client public key with our CA"],
    ["$exec", "openssl x509 -req -days \"${validity_days}\" -sha256 -in \"${dir}/${files.client_csr}\" -CA \"${dir}/${files.ca}\" -CAkey \"${dir}/${files.ca_key}\" -CAcreateserial -out \"${dir}/${files.client_cert}\" -extfile \"${dir}/${files.client_ext}\"  -passin pass:${password}"],
    ["$ensureFile", "${dir}/${files.server_cert}"],
  ],
  "gen_client": [
    "gen_client_key",
  ...
  "cleanup": ['series',
    ...
    ["$exec", "rm \"${dir}/${files.server_csr}\" \"${dir}/${files.client_csr}\" \"${dir}/${files.server_ext}\" \"${dir}/${files.client_ext}\""],
    ["$ensureNoFile",
  ...
  "chmod": ['series',
    ...
    ["$exec", "chmod -v 0400 \"${dir}/${files.ca_key}\" \"${dir}/${files.client_key}\" \"${dir}/${files.server_key}\""],
    ["$print", "* Make certificates world-readable (no write access)"],
    ["$exec", "chmod -v 0444 \"${dir}/${files.ca}\" \"${dir}/${files.server_cert}\" \"${dir}/${files.client_cert}\""],
  ],
  "prepare": [
    "$exec", "mkdir -p ${dir}"
  ],
...
const opemSslActivity: FullConfig = {
  base_dir: './demo',
  version: '2.5.22',
  actions: {
    ...opensslActions,
