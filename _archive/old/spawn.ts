...
//import {spawn, ChildProcessByStdio, SpawnOptionsWithStdioTuple} from 'child_process'
...
const printChunk = (data, needPrint: 'json') => {
...
export class Cmd {
  ...
  _execute(pathname, params, options, exitCallback) {
    ...
    try {
      child /* : ChildProcessByStdio<Writable, Readable, Readable> */ = spawn(pathname, params, spawnOptions);
    } catch (e) {
  ...
  async _spawnExecutePromised(cmd: string|string[], params, options?): Promise<string> {
    ...
    console.log('* COMMAND: "' + cmd + ' ' + params.join(' ')+'"');
...
export function Spawn (config?: CmdConfig) {
